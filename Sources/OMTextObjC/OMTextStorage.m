...
#import "OMTextStorage.h"


@interface OMTextStorage ()

@property (nonatomic) BOOL hasProcessedEdit;

@end

@implementation OMTextStorage
...
- (void)replaceCharactersInRange:(NSRange)range withString:(NSString *)str {
    ...
    if ([(NSObject*)self.storageDelegate respondsToSelector:@selector(textStorage:didReplaceCharactersInRange:withString:)]) {
...
- (void)processEditing {
    ...
    if ([self.storageDelegate respondsToSelector:@selector(textStorageWillBeginProcessingEdit:)]) {
        [self.storageDelegate textStorageWillBeginProcessingEdit:self ];
    }
